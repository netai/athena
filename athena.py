...
from client import athenapath
from client import diagnose
from client.conversation import Conversation

# Add athenapath.LIB_PATH to sys.path
sys.path.append(athenapath.LIB_PATH)
        ...
        if not os.path.exists(athenapath.CONFIG_PATH):
            try:
                ...
                self._logger.error("Could not create config dir: '%s'",
                                   athenapath.CONFIG_PATH, exc_info=True)
                raise

        # Check if config dir is writable
        if not os.access(athenapath.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. athena " +
                                  "won't work correctly.",
                                  athenapath.CONFIG_PATH)

        # FIXME: For backwards compatibility, move old config file to newly
        #        created config dir
        old_configfile = os.path.join(athenapath.LIB_PATH, 'profile.yml')
        new_configfile = athenapath.config('profile.yml')
